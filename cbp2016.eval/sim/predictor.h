...
bool LowConf[USE_PARALLEL];
bool HighConf[USE_PARALLEL];
bool AltConf[USE_PARALLEL];
bool MedConf[USE_PARALLEL];
bool alttaken[USE_PARALLEL];			// alternate  TAGEprediction
bool tage_pred[USE_PARALLEL];			// TAGE prediction
bool LongestMatchPred[USE_PARALLEL];
int HitBank[USE_PARALLEL];			// longest matching bank
int AltBank[USE_PARALLEL];			// alternate matching bank
int Seed[USE_PARALLEL];			// for the pseudo-random number generator
int8_t BIM[USE_PARALLEL];
bool pred_inter[USE_PARALLEL];
...
//#define INDUSEALT 0
int8_t use_alt_on_na[SIZEUSEALT][USE_PARALLEL];




int TICK[USE_PARALLEL];			// for the reset of the u counter
...
bentry *btable[USE_PARALLEL];			//bimodal TAGE table
gentry *gtable[USE_PARALLEL][NHIST + 1];	// tagged TAGE tables
int m[NHIST + 1];
int TB[NHIST + 1];
int logg[NHIST + 1];
...
bool pred_taken[USE_PARALLEL];		// prediction
...
public:
  ...
  void reinit (int k)
  {
    ...
    gtable[k][1] = new gentry[NBBANK[0] * (1 << LOGG)];
    gtable[k][BORN] = new gentry[NBBANK[1] * (1 << LOGG)];
    for (int i = BORN + 1; i <= NHIST; i++)
      gtable[k][i] = gtable[k][BORN];
    for (int i = 2; i <= BORN - 1; i++)
      gtable[k][i] = gtable[k][1];
    btable[k] = new bentry[1 << LOGB];
    for (int i = 1; i <= NHIST; i++)
      {
...
// gindex computes a full hash of PC, ghist and phist
  ...
  bool getbim ( uint i)
  {
    BIM[i] = (btable[i][BI].pred << 1) + (btable[i][BI >> HYSTSHIFT].hyst);
    HighConf[i] = (BIM[i] == 0) || (BIM[i] == 3);
    LowConf[i] = !HighConf[i];
    AltConf[i] = HighConf[i];
    MedConf[i] = false;
    return (btable[i][BI].pred > 0);
  }

  void baseupdate (uint i, bool Taken)
  {
    int inter = BIM[i];
    if (Taken)
      {
	if (inter < 3)
      ...
      inter--;
    btable[i][BI].pred = inter >> 1;
    btable[i][BI >> HYSTSHIFT].hyst = (inter & 1);
  };
//just a simple pseudo random number generator: use available information
  ...
  int MYRANDOM (int i)
  {
    Seed[i]++;
    Seed[i] ^= phist;
    Seed[i] = (Seed[i] >> 21) + (Seed[i] << 11);
    Seed[i] ^= ptghist;
    Seed[i] = (Seed[i] >> 10) + (Seed[i] << 22);
    Seed[i] ^= GTAG[BORN + 2];
    return (Seed[i]);
  };
  ...
  void Tagepred (UINT64 i,UINT64 PC)
  {
    HitBank[i] = 0;
    AltBank[i] = 0;
    for (int j = 1; j <= NHIST; j += 2)
      {
	GI[j] = gindex (PC, j, phist, ch_i);
	GTAG[j] = gtag (PC, j, ch_t[0], ch_t[1]);
	GTAG[j + 1] = GTAG[j];
	GI[j + 1] = GI[j] ^ (GTAG[j] & ((1 << LOGG) - 1));
      }

    uint T = (PC ^ (phist & ((1 << m[BORN]) - 1))) % NBBANK[1];
    for (int j = BORN; j <= NHIST; j++)
      if (NOSKIP[j])
	{
	  GI[j] += (T << LOGG);
	  T++;
...
//just do not forget most address are aligned on 4 bytes
    ...
    {
      alttaken[i] = getbim (i);
	  tage_pred[i] = alttaken[i];
	  LongestMatchPred[i] = alttaken[i];
	  
    //   for(int j =0;j<USE_PARALLEL;j++)
    //   {
    //     tage_pred[j] = alttaken[j];
    //     LongestMatchPred[j] = alttaken[j];
    //   }
    }
...
//Look for the bank with longest matching history
    for (int j = NHIST; j > 0; j--)
      {
        if (NOSKIP[j])
          if (gtable[i][j][GI[i]].tag == GTAG[i])
            {
              HitBank[i] = j;
              LongestMatchPred[i] = (gtable[i][HitBank[i]][GI[HitBank[i]]].ctr >= 0);
              break;
            }
      }

//Look for the alternate bank
    for (int j = HitBank[i] - 1; j > 0; j--)
      {
	if (NOSKIP[j])
	  if (gtable[i][j][GI[j]].tag == GTAG[j])
	    {
...
//computes the prediction and the alternate prediction
	...
	if (AltBank[i] > 0)
	  {
	    alttaken[i] = (gtable[i][AltBank[i]][GI[AltBank[i]]].ctr >= 0);
	    AltConf[i] = (abs (2 * gtable[i][AltBank[i]][GI[AltBank[i]]].ctr + 1) > 1);
	  }
...
//USE_ALT_ON_NA is positive  use the alternate prediction
	bool Huse_alt_on_na = (use_alt_on_na[((((HitBank[i]-1)/8)<<1)+AltConf[i])][i] >= 0);
	if ((!Huse_alt_on_na)
	    || (abs (2 * gtable[i][HitBank[i]][GI[HitBank[i]]].ctr + 1) > 1))
	  tage_pred[i] = LongestMatchPred[i];
	else
	  tage_pred[i] = alttaken[i];
	HighConf[i] =
	  (abs (2 * gtable[i][HitBank[i]][GI[HitBank[i]]].ctr + 1) >=
	   (1 << CWIDTH) - 1);
	LowConf[i] = (abs (2 * gtable[i][HitBank[i]][GI[HitBank[i]]].ctr + 1) == 1);
	MedConf[i] = (abs (2 * gtable[i][HitBank[i]][GI[HitBank[i]]].ctr + 1) == 5);
      }
...
// computes the TAGE table addresses and the partial tags
	for(int i=0;i<USE_PARALLEL;i++)
    	Tagepred (i,PC);
	for(int i=0;i<USE_PARALLEL;i++)
		pred_taken[i] = tage_pred[i];
...
// PREDICTOR UPDATE
  ...
  void UpdatePredictor (UINT64 PC, OpType opType,
			bool resolveDir, bool *predDir, UINT64 branchTarget, uint index)
  {
	  // 只针对我的这一条指令的情况进行更新，其他的都看作是不跳转的指令。
	  // 最简单的直接更新一条指令，其他指令都不更新。
	  for (int i=0;i<USE_PARALLEL;i++)
	  {
		  if (i == (index -1))
		  {
			UpdatePredictor_mini(i,PC+(i*4),opType,resolveDir,*(predDir+i),branchTarget);
		  }
	  }
  }


  void UpdatePredictor_mini (int i, UINT64 PC, OpType opType,
...
//TAGE UPDATE
    bool ALLOC = ((tage_pred[i] != resolveDir) & (HitBank[i] < NHIST));
    if (pred_taken[i] == resolveDir)
      if ((MYRANDOM (i) & 31) != 0)
	ALLOC = false;
...
// an entry is considered as newly allocated if its prediction counter is weak
	if (PseudoNewAlloc)
	  {
	    if (LongestMatchPred[i] == resolveDir)
	      ALLOC = false;
// if it was delivering the correct prediction, no need to allocate a new entry
//even if the overall prediction was false
	    if (LongestMatchPred[i] != alttaken[i])
	      {
		ctrupdate (use_alt_on_na[((((HitBank[i]-1)/8)<<1)+AltConf[i])][i],
			   (alttaken[i] == resolveDir), ALTWIDTH);
	      }
	...
	int TruePen = 0;
	int NA = 0;
	int DEP = ((((HitBank[i] - 1 + 2 * A) & 0xffe)) ^ (MYRANDOM (i) & 1));
...
#define DIPINSP
		      ...
		      gtable[i][j][GI[j]].u = ((MYRANDOM (i) & 31) == 0);
// protect randomly from fast replacement
#endif
		      gtable[i][j][GI[j]].tag = GTAG[j];
		      gtable[i][j][GI[j]].ctr = (resolveDir) ? 0 : -1;
		      NA++;
		      if (T <= 0)
			{
			  break;
		  ...
		  {
#ifdef DIPINSP
		    if ((gtable[i][j][GI[j]].u ==
			 1) & (abs (2 * gtable[i][j][GI[j]].ctr + 1) == 1))
		      {
			if ((MYRANDOM (i) & 7) == 0)
			  gtable[i][j][GI[j]].u = 0;
		      }
	...
	if (TICK[i] >= BORNTICK)
	  {
	    ...
	    for (int j = 1; j <= BORN; j += BORN - 1)
	      for (int k = 0; k <= NBBANK[j / BORN] * (1 << logg[j]) - 1; k++)
		{
...
//update predictions
    if (HitBank[i] > 0)
      {
	if (abs (2 * gtable[i][HitBank[i]][GI[HitBank[i]]].ctr + 1) == 1)
	  if (LongestMatchPred[i] != resolveDir)
	    {			// acts as a protection 
	      if (AltBank[i] > 0)
		{
		  if (abs (2 * gtable[i][AltBank[i]][GI[AltBank[i]]].ctr + 1) == 1)
		    gtable[i][AltBank[i]][GI[AltBank[i]]].u = 0;
		  //just mute from protected to unprotected
		  ctrupdate (gtable[i][AltBank[i]][GI[AltBank[i]]].ctr,
			     resolveDir, CWIDTH);
		  if (abs (2 * gtable[i][AltBank[i]][GI[AltBank[i]]].ctr + 1) == 1)
		    gtable[i][AltBank[i]][GI[AltBank[i]]].u = 0;
		}
	      if (AltBank[i] == 0)
		baseupdate (i,resolveDir);
	    }
	if (abs (2 * gtable[i][HitBank[i]][GI[HitBank[i]]].ctr + 1) == 1)
	  gtable[i][HitBank[i]][GI[HitBank[i]]].u = 0;
//just mute from protected to unprotected
	ctrupdate (gtable[i][HitBank[i]][GI[HitBank[i]]].ctr, resolveDir, CWIDTH);
//sign changes: no way it can have been useful
	if (abs (2 * gtable[i][HitBank[i]][GI[HitBank[i]]].ctr + 1) == 1)
	  gtable[i][HitBank[i]][GI[HitBank[i]]].u = 0;
	if (alttaken[i] == resolveDir)
	  if (AltBank[i] > 0)
	    if (abs (2 * gtable[i][AltBank[i]][GI[AltBank[i]]].ctr + 1) == 7)
	      if (gtable[i][HitBank[i]][GI[HitBank[i]]].u == 1)
		{
		  if (LongestMatchPred[i] == resolveDir)
		    {
		      gtable[i][HitBank[i]][GI[HitBank[i]]].u = 0;
      ...
      if (LongestMatchPred[i] == resolveDir)
	{
	  ...
	  if (gtable[i][HitBank[i]][GI[HitBank[i]]].u < (1 << UWIDTH) - 1)
	    gtable[i][HitBank[i]][GI[HitBank[i]]].u++;
	}
//END TAGE UPDATE
